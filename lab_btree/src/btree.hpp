...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    if (subroot->is_leaf) {
        auto itr  = subroot->elements.begin();
        subroot->elements.insert(itr + first_larger_idx, pair);
    } else {
        BTreeNode* insertNode = subroot->children[first_larger_idx];
        insert(insertNode, pair);
        if (insertNode->elements.size() >= order) {
