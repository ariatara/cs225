...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    size_t lower = 0;
    size_t upper = elements.size() - 1;

    if (val < elements[lower]) {
        return 0;
    } else if (val > elements[upper]) {
        return upper + 1;
    }
    ...
    if (elements.empty() || val < elements[0] || val == elements[0]) {
        return 0;
    }

    for (unsigned i = 1; i < elements.size(); i++) {
        if (val > elements[i - 1] && (val < elements[i] || val == elements[i])) {
            return i;
        }
    }
