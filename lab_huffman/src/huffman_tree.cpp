                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleNode == NULL && mergeNode == NULL) {
        return NULL;
    } else if (singleNode != NULL && mergeNode == NULL) {
        singleQueue.pop();
        return singleNode;
    } else if (singleNode == NULL && mergeNode != NULL) {
        mergeQueue.pop();
    ...
    if ((singleNode->freq).getFrequency() <= (mergeNode->freq).getFrequency()) {
        singleQueue.pop();
        return singleNode;
    } else {
        mergeQueue.pop();
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    for (Frequency frequency : frequencies) {
        TreeNode* node = new TreeNode(frequency);
        singleQueue.push(node);
    }

    while (singleQueue.size() + mergeQueue.size() > 1) {
        TreeNode* firstNode = removeSmallest(singleQueue, mergeQueue);
        TreeNode* secondNode = removeSmallest(singleQueue, mergeQueue);

        int firstFrequency = (firstNode->freq).getFrequency();
        int secondFrequency = (secondNode->freq).getFrequency();

        TreeNode* internalNode = new TreeNode(firstFrequency + secondFrequency);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        if (bfile.getNextBit()) {
            ...
            Frequency frequency(character, 0);
