...
class KDTree
{
  ...
  public:
     ...
     */
    KDTree(const vector<Point<Dim>>& newPoints); // store
  ...
  private:
    ...
    KDTreeNode *root;
    size_t size;
    vector<Point<Dim>> input_points_;
    ...
    int partition(int startIndex, int endIndex, int pivotIndex, int dimension); // called by select

    void select(int startIndex, int endIndex, int pivotIndex, int dimension); // called by buildTree, recursive

    KDTreeNode* buildTree(int startIndex, int endIndex, int dimension); // called by constructor, recursive

    Point<Dim> findNearestNeighbor(KDTreeNode* subTreeRoot, const Point<Dim>& query, int dimension) const;

    void clear(KDTreeNode* subTreeRoot);

    KDTreeNode* copy(KDTreeNode* subTreeRoot, KDTreeNode* otherSubTreeRoot);
};
