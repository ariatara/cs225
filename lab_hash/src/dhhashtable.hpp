...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    elems++;
    unsigned currentHash = hashes::hash(key, size);
    unsigned secondHash = secondary_hash(key, size);

    while (table[currentHash] != NULL) {
        currentHash = (currentHash + secondHash) % size;
    }

    table[currentHash] = new pair<K, V>(key, value);
    should_probe[currentHash] = true;
...
void DHHashTable<K, V>::remove(K const& key)
{
    ...
    while (table[currentHash] != NULL && table[currentHash]->first != key) {
        currentHash = (currentHash + secondHash) % size;
    }

    if (table[currentHash] == NULL) {
        return;
    } else if (table[currentHash]->first == key) {
        delete table[currentHash];
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    unsigned currentHash = hashes::hash(key, size);
    unsigned secondHash = secondary_hash(key, size);
    unsigned start = currentHash;
    
    while (should_probe[currentHash]) {
        if (table[currentHash] != NULL) {
            if (table[currentHash]->first == key) {
                return currentHash;
            }
        }

        currentHash = (currentHash + secondHash) % size;
        if (start == currentHash) {
            break;
        }
    }
