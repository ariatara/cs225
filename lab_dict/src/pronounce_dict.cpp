...
{
    /* Your code goes here! */
    string upperCaseWord1 = word1;
    string upperCaseWord2 = word2;

    for (char& character : upperCaseWord1) {
        character = toupper(character);
    }
    for (char& character : upperCaseWord2) {
        character = toupper(character);
    }

    map<string, vector<string>>::const_iterator it1 = dict.find(upperCaseWord1);
    map<string, vector<string>>::const_iterator it2 = dict.find(upperCaseWord2);

    if (it1 == dict.end() || it2 == dict.end()) {
        return false;
    }

    vector<string> value1 = (*it1).second;
    vector<string> value2 = (*it2).second;
